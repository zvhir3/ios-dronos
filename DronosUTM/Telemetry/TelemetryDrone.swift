...
class KeyedInterfaceViewController: UIViewController {
    ...
    @IBAction func getBatteryLevel(_ sender: Any) {
        ...
        DJISDKManager.keyManager()?.getValueFor(batteryLevelKey!, withCompletion: { [unowned self] (value: DJIKeyedValue?, error: Error?) in
            ...
            self.showToast(message: "\(value!.unsignedIntegerValue) %", font: .systemFont(ofSize: 12.0))
            return
        })
    }
    ...
    @IBAction func startStopListeningCoordinates(_ sender: Any) {
        ...
        if isListening {
            ...
            self.listeningCoordinatesLabel.text = "Stopped fetching telemetry..";
            ...
            DJISDKManager.keyManager()?.startListeningForChanges(on: locationKey!, withListener: self, andUpdate: { (oldValue: DJIKeyedValue?, newValue: DJIKeyedValue?) in
                if newValue != nil {
                    ...
                    DJISDKManager.keyManager()?.getValueFor(batteryLevelKey!, withCompletion: { [unowned self] (value: DJIKeyedValue?, error: Error?) in
                        ...
                        self.listeningCoordinatesLabel.text = "Lat: \(aircraftCoordinates.coordinate.latitude) Long: \(aircraftCoordinates.coordinate.longitude) Alt: \(aircraftCoordinates.altitude) Speed: \(aircraftCoordinates.speed) Timestamp: \(aircraftCoordinates.timestamp)"
            ...
            self.listeningCoordinatesLabel.text = "Started fetching telemetry..";
