...
/**
 *  Flight controller flight modes. If the current flight mode means P-GPS, it will
 *  show OPTI in DJI app if the vision system is working. If the current flight Mode
 *  contains GPS and also the RTK signal is healthy, it will show RTK in DJI app.
 */
typedef NS_ENUM (NSUInteger, DJIFlightMode){
    ...
    /**
     *  The aircraft should move following the terrain. Shown as TerrainTracking in DJI
     *  app.
     */
    ...
    /**
     *  Active track mode, corresponds to Spotlight active track mode. Shown as
     *  QuickShot/ActiveTrack in DJI app.
     */
...
/**
 *  An enum used to identify the different stages of the go-home command. Some
 *  aircraft only support part of the state depending on the firmware.
 *   For exampele:
 *   Phantom 4 Pro V2.0 and Mavic 2 Enterprise Dual only support
 *  `DJIGoHomeExecutionStateGoDownToGround`.
 *   Matrice 300 RTK only suppoort `DJIGoHomeExecutionStateGoUpToHeight`,
 *  `DJIGoHomeExecutionStateAutoFlyToHomePoint` and
 *  `DJIGoHomeExecutionStateGoDownToGround`.
 */
...
typedef struct
{
     ...
     *  `DJIVirtualStickVerticalControlMode` to set velocity or altitude mode.
     */
...
/*********************************************************************************/
#pragma mark DJINavigationSatelliteSystem
/*********************************************************************************/
...
typedef NS_ENUM (uint8_t, DJINavigationSatelliteSystem){
    ...
    /**
     *  Using GSP and GLONSS satellite data.
     */
    DJINavigationSatelliteSystemGPSAndGLONASS,
 

    /**
     *  Only use Beidou satellite navigation system.
     */
    DJINavigationSatelliteSystemBeiDou,
    
   
    /**
     *  Unknown.
     */
    DJINavigationSatelliteSystemUnknown = 0xFF,
};
...
typedef NS_ENUM(uint8_t, DJIFlightAssistantObstacleSensingDirection) {
    ...
    /**
     *  Upward sensing. The upward distance range is 1.1m~30m
     */
    DJIFlightAssistantObstacleSensingDirectionUpward,
    

    /**
     *  Downward sensing. The downward distance range is 0.6m~30m
     */
    DJIFlightAssistantObstacleSensingDirectionDownward,
    

    /**
     *  Horizontal Field. The horizontal distance range is 1.1m~40m
     */
    DJIFlightAssistantObstacleSensingDirectionHorizontal,
    

    /**
     *  Unknown sensing direction.
     */
    DJIFlightAssistantObstacleSensingDirectionUnknown = 0xFF,
};
...
/**
 *  `YES` if the beacons on the top and bottom are turned on. It is supported by
 *  Matrice 300 RTK. The other products will ignore this property.
 */
...
/**
 *  `YES` if the beacon is turned on. It is only valid for Matrice 300 RTK.
 */
@property (nonatomic, readwrite) BOOL beaconOn;
