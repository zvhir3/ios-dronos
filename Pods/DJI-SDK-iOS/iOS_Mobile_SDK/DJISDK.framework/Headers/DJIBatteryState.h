 ...
 *  will start to preserve the temperature and maintain it between 6 Celsius degrees
 *  and 8 Celsius degrees.
 */
...
typedef NS_ENUM(uint8_t, DJISmartBatterySOHState) {
    DJISmartBatterySOHState_Normal = 0,      // 正常
    DJISmartBatterySOHState_Alert = 1, // 循环次数预警
    DJISmartBatterySOHState_Warning = 2, // 循环次数告警
};
...
typedef NS_ENUM(uint8_t, DJISmartBatteryType) {
    ...
    /**
     *  The smart battery type is other.
     */
    DJISmartBatteryTypeOther = 0,
    


    /**
     *  The smart battery type is Mavic Mini Japan, which is legal to use in Japan.
     */
    DJISmartBatteryTypeMavicMiniJapan = 1,
    

    /**
     *  The smart battery type is unknown.
     */
    DJISmartBatteryTypeUnknown = 0xFF,
};
...
/**
 *  Returns the design capacity of the battery in mAh (milliamp hours). It is the
 *  ideal capacity when the battery is new. This value will not change over time.
 *  It is only supported by smart battery.
 */
...
@property (nonatomic, readonly) DJISmartBatterySOHState cycleState;

@property(nonatomic, readonly) int cycleLimitTime;
