...
typedef NS_ENUM (NSUInteger, DJIWaypointActionType){
    ...
    /**
     *  Rotates the gimbal's pitch. The range of the pitch angle should be gotten from
     *  `DJIGimbalParamAdjustPitch` or `DJIGimbalParamAdjustPitchUpwards`.
     */
 ...
 *  aircraft is relative to  the ground at the take-off location, and should not be
 *  larger than the aircraft's maximum altitude. If  two adjacent waypoints have
 *  different altitudes, the altitude will gradually change as the aircraft flies
 *  between waypoints.
 */
 ...
 *  [-180, 180] degrees, where 0 represents True North. `heading` relate to behavior
 *  between this waypoint and the next waypoint in the mission.
 */
 ...
 *  changing its heading. `turnMode` relate to behavior between this waypoint and
 *  the next waypoint in the mission. Not supported by Mavic 2 Series and Phantom 4
 *  RTK.
 */
 ...
 *  `DJIWaypointMission` property `rotateGimbalPitch` is YES .  The range of the
 *  pitch angle should be gotten from the `DJIGimbalParamAdjustPitch` or
 *  `DJIGimbalParamAdjustPitchUpwards`.
 */
 ...
 *   2) 'speed'
 *   3) <code>DJIWaypointMissionOperator_setAutoFlightSpeed</code>
 *    4) `autoFlightSpeed`
 ...
 *  In comparison. Only supported by flight controller firmware 3.2.10.0 or above.
 */
