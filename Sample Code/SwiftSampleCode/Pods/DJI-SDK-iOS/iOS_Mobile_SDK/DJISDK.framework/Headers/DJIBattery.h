...
/**
 *  Gets the manufacture date of current battery. It it only supported by Matrice
 *  300 RTK.
 *  
 ...
 *  Not supported by Mavic Air 2, DJI Air 2S, non-smart batteries, Spark and the
 *  Osmo series.
 *  
...
/**
 *  Sets the behavior that will be executed when the cell voltage crosses under the
 *  Level 2 threshold. It is not supported by Mavic Air.
 *  
 *  @param completion Asynchronous execution result block that contains the voltage threshold behavior.
 *  @param behavior Aircraft behavior when voltage threshold is crossed.
 *  @param error Error retrieving the value.
...
/**
 *  Enables/disables the LED lights on the batteries. If all batteries' LEDs on the
 *  aircraft are turned off, the aircraft's power button LED indicator should be
 *  off. It's supported only by Matrice 300 RTK.
 *  
 *  @param enabled `YES` to enable the LED lights on the batteries.
 *  @param completion Completion block that receives the setter execution result.
 */
...
/**
 *  Determines if the LEDs on the battery is on. If all batteries' LEDs on the
 *  aircraft are turned off, the aircraft's power button LED indicator should be
 *  off. It's supported only by Matrice 300 RTK.
 *  
