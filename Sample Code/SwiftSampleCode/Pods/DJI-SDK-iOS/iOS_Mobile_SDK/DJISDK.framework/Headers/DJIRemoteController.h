 ...
 *  @param batteryState Current state of the Remote Controller's battery.
 */
...
/**
 *  Delegate method that receives the calibration progress of the remote controller.
 *  The remote controller will start to update the progress after the calibration is
 *  started successfully.
 *  
 *  @param rc The remote controller.
 *  @param progress The calibration progress of the remote controller.
 */
- (void)remoteController:(DJIRemoteController *)rc didUpdateCalibrationProgress:(DJIRCCalibrationProgress *)progress;


/**
 *  Delegate method to receive the latest pairing state from the remote controller
 *  that supporting multi-device pairing.
 *  
 *  @param rc Remote controller that updates the pairing state.
 *  @param state The latest pairing state.
 */
- (void)remoteController:(DJIRemoteController *)rc didUpdateMultiDevicePairingState:(DJIRCMultiDeviceAggregationState *)state;


/**
 *  Delegate method to receive the current RTK channel enabled from the remote
 *  controller that supporting multi-device pairing.
 *  
 *  @param rc Remote controller that updates the pairing state.
 *  @param enabled The current RTK channel enabled.
 */
...
/*********************************************************************************/
#pragma mark Multi-Device pairing
/*********************************************************************************/

/**
 *  Determines if multi-device pairing supported by the remote controller. When the
 *  remote controller supports multi-device pairing, it can be paired to more than
 *  one device (e.g. aircraft, RTK base station) at the same time. When this feature
 *  is supported, use `startMultiDevicePairingWithTarget:withCompletion` and
 *  `stopMultiDevicePairingWithCompletion` to start and stop pairing instead of
 *  using the following methods:
 *   - `startPairingWithCompletion`
 *   - `stopPairingWithCompletion`
 *   - `getPairingStateWithCompletion`. This feature is only supported by Phantom 4
 *  RTK.
 *  
 *  @return `YES` if multi-device pairing is supported.
 */
 ...
 *  This paring method is for Phantom 4 RTK only. To use RTK feature with a base
 *  station, the Phantom 4 RTK remote controller need to pair both the base station
 *  and the aircraft at the same time.
 *   CAUTION: pairing an RTK base station will unpair the paired aircraft.
 *  Therefore, to use the base station properly, the user needs to pair the base
 *  station first, and the aircraft after. It is only supported when
 *  `isMultiDevicePairingSupported` returns `YES`.
 *  
 *  @param target The device target to start multi-device pairing.
 *  @param completion Completion block.
 */
...
/**
 *  Stops the multi-device pairing. It is used after the pairing is started and
 *  before the remote controller is paired to a device successfully. It is not
 *  necessary to call this method if the remote controller is paired to a device
 *  successfully. Only Supported by Phantom 4 RTK.
 *  
 *  @param completion Completion block.
 */
...
/*********************************************************************************/
#pragma mark RTK Channel
/*********************************************************************************/


/**
 *  Enables the RTK channel on the remote controller. When RTK air system is using
 *  the base station as the reference station, the remote controller should enable
 *  the RTK channel and reserve some bandwidth for data transmission between the
 *  base station and the air system.
 *   CAUTION: toggling RTK channel will reset the airlink between the remote
 *  controller and the aircraft. The airlink may take 3 seconds to recover. DO NOT
 *  enable/disable the channel when the aircraft is flying. If the base station is
 *  reconnected, you need to call this interface to enabled the RTK channel again.
 *  Only Supported by Phantom 4 RTK.
 *  
 *  @param enabled `YES` to enable the RTK channel on the remote controller.
 *  @param completion Completion block.
 */
...
/**
 *  Sets which of the gimbal directions the top left dial (Gimbal Dial) on the
 *  Remote Controller will control. The three options (pitch, roll, and yaw) are
 *  outlined in the enum named `DJIRCGimbalAxis`.
 *  
 *  @param axis Gimbal direction to be set that the top left dial on the Remote Controller will control.
 *  @param completion Completion block.
 */
...
/**
 *  Enables invoking shooting photo interface when the shutter button is pressed.
 *  Mavic 2 series remote controller's shutter button will not work physically when
 *  connecting to Mobile SDK. This setting will be stored in software application
 *  localized storage paired with the RC SN key, which means, if the application is
 *  killed, it will be set to enabled by default when the application is relaunched.
 *  It is only supported by Mavic 2 series.
 *  
 *  @param enabled `YES` to enable invoking shooting photo interface when the shutter button is pressed.
 *  @param completion Completion block.
 */
...
/**
 *  Gets if shooting photo interface is involved when the shutter button is pressed.
 *  It is only supported by Mavic 2 series.
 *  
 *  @param enabled If shooting photo interface is invoking when the shutter button is pressed.
 *  @param error Error retrieving the value.
 *  @param completion Completion block.
 */
...
/**
 *  Enables invoking video record interface when the record button is pressed. Mavic
 *  2 series remote controller's record button will not work physically when
 *  connecting to Mobile SDK. This setting will be stored in software application
 *  localized storage paired with the RC SN key. Only supported by Mavic 2 series.
 *  
 *  @param enabled `YES` to enable invoking video record interface when the record button is pressed.
 *  @param completion Completion block.
 */
...
/**
 *  Gets if video record interface is involved when the record button is pressed. It
 *  is only supported by Mavic 2 series.
 *  
 *  @param enabled If video record interface is invoking when the record button is pressed.
 *  @param error Error retrieving the value.
 *  @param completion Completion block.
 */
...
/**
 *  `YES` to switch camera flat mode between photo and video when the
 *  `photoAndVideoToggleButton` button is pressed. `NO` when
 *  `photoAndVideoToggleButton` button is pressed, we will not bind any event. You
 *  can monitor the button and handle custom events by yourself. This setting will
 *  be stored in software application localized storage paired with the RC SN key,
 *  which means. Only supported by Mavic Air 2, DJI Air 2S.
 *  
 *  @param enabled `YES` to switch camera flat mode between photo and video when the `photoAndVideoToggleButton` button is pressed.
 *  @param completion Completion block.
 */
...
/**
 *  Gets whether switching camera flat mode between photo and video is enabled when
 *  the `photoAndVideoToggleButton` button is pressed. It is only supported by Mavic
 *  Air 2.
 *  
 *  @param enabled Gets the enable value.
 *  @param error Error retrieving the value.
 *  @param completion Completion block.
 */
...
/**
 *  `YES` to enable invoking shoot photo or record interface when the record button
 *  is pressed depend on the `photoAndVideoToggleButton` button state. `NO` when
 *  `photoAndVideoToggleButton` button is pressed, we will not bind any event. You
 *  can monitor the button and handle custom events by yourself. This setting will
 *  be stored in software application localized storage paired with the RC SN key.
 *  Only supported by Mavic AIr 2, DJI Air 2S.
 *  
 *  @param enabled `YES` to enable invoking shoot photo or record interface when the record button is pressed depend on the `photoAndVideoToggleButton` button state.
 *  @param completion Completion block.
 */
...
/**
 *  Gets if shoot photo or video record interface is involved when the
 *  `photoAndVideoToggleButton` button is pressed. It is only supported by Mavic Air
 *  2, DJI Air 2S.
 *  
 *  @param enabled Gets the enable value.
 *  @param error Error retrieving the value.
 ...
 *  600 Pro, A3 and N3) and Inspire 2. It's not supported by Matrice 300 RTK.
 *  
 ...
 *  `isCustomizableButtonSupported` returns `YES`. Supported by the Cendence remote
 *  controller.
 *  
 ...
 *  valid when `isCustomizableButtonSupported` returns `YES`. Supported by the
 ...
 *  index can only be set to 0. It is only supported by M210, M210 RTK and Matrice
 *  300 RTK.
 *  
...
/**
 *  Gets the hardware components (sticks or dials) on the remote controller that
 *  requires calibration. After starting the calibration, user should move the
 *  components returned by this method to their physical limits to complete the
 *  calibration. It is only supported by Phantom 4 RTK.
 *  
 *  @param components The hardware components requiring calibration. Each element in the array is a value of `DJIRCCalibrationComponent`.
 *  @param error Error retrieving the value.
 *  @param completion Completion block to receive the result.
 */
...
/**
 *  Starts the calibration for the remote controller. Before starting the
 *  calibration, all the hardware components requiring calibration (see
 *  `getCalibrationComponentsWithCompletion`) should be returned to their midpoints.
 *  The calibration progress for each component can be received from
 *  `remoteController:didUpdateCalibrationProgress`. It is only supported by Phantom
 *  4 RTK.
 *  
 *  @param completion Completion block to receive the result.
 */
 ...
 *   CAUTION: stopping the calibration before each hardware component returns its
 *  physical limits at least once may cause error in the remote controller. Resolve
 *  the error by restarting the calibration. It is only supported by Phantom 4 RTK.
 *  
 *  @param completion Completion block to receive the result.
 */
...
/**
 *  Return whether secondary video output supported. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 *  @return A boolean value.
 */
 ...
 *  video stream to one of the Remote Controller's physical HDMI and SDI ports. It
 *  is supported by Phantom 4 Professional, Matrice 200, Matrice 200 series V2,
 ...
 *  `setSecondaryVideoOutputPort:withCompletion` to choose the port. It is supported
 *  by Phantom 4 Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and
 *  Inspire 2.
 *  
 ...
 *  Only one port can be active at once. It is supported by Phantom 4 Professional,
 *  Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 ...
 *  possible. Only one port can be active at once. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 *  @param port The value of the `DJIRCSecondaryVideoOutputPort` Enum.
 *  @param error Error retrieving the value.
 *  @param completion Completion block.
 */
 ...
 *  remote controller module can connect to both an FPV camera (through the HDMI and
 *  AV ports) and a camera mounted on the HD Gimbal (through the Gimbal port). The
 *  output video can then be a combination of the two video sources. Either a single
 *  video source can be  displayed, or one can be displayed within the other (as a
 *  Picture in Picture, or PIP). If the mode is set incorrectly,  then no output
 *  video will be displayed. It is supported by Phantom 4 Professional, Matrice 200,
 *  Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 ...
 *  in Picture, or PIP). It is supported by Phantom 4 Professional, Matrice 200,
 *  Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 *  @param videoDisplayMode The value of the `DJIRCSecondaryVideoDisplayMode` Enum.
 *  @param error Error retrieving the value.
 *  @param completion Completion block to receive the result.
 */
 ...
 *  video. It is supported by Phantom 4 Professional, Matrice 200, Matrice 200
 *  series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Sets the OSD top margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Gets the OSD top margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Sets the OSD left margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Gets the OSD left margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Sets the OSD bottom margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Gets the OSD bottom margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Sets the OSD right margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Gets the OSD right margin in video pixels. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Sets the OSD units to either metric or imperial. It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
...
/**
 *  Gets the OSD units (metric or imperial). It is supported by Phantom 4
 *  Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 *  @param unit The value of the `DJIRCSecondaryVideoOSDUnit` Enum.
 *  @param error Error retrieving the value.
 *  @param completion Completion block to receive the result.
 */
...
/**
 *  Sets the Remote Controller HDMI video port output video format. It is supported
 *  by Phantom 4 Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and
 *  Inspire 2.
 *  
 *  @param format The value of the `DJIRCSecondaryVideoFormat` Enum.
 *  @param completion Completion block to receive the result.
 */
...
/**
 *  Gets the Remote Controller HDMI video port output video format. It is supported
 *  by Phantom 4 Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and
 *  Inspire 2.
 *  
 *  @param format The value of the `DJIRCSecondaryVideoFormat` Enum.
...
/**
 *  Sets the Remote Controller SDI video port output video format. It is supported
 *  by Phantom 4 Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and
 *  Inspire 2.
 *  
 *  @param format The value of the `DJIRCSecondaryVideoFormat` Enum.
...
/**
 *  Gets the Remote Controller SDI video port output video format. It is supported
 *  by Phantom 4 Professional, Matrice 200, Matrice 200 series V2, Inspire 1 and
 *  Inspire 2.
 *  
 *  @param format The value of the `DJIRCSecondaryVideoFormat` Enum.
 ...
 *  the main subject video feed. It is supported by Phantom 4 Professional, Matrice
 *  200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 ...
 *  200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 *  @param position The value of the `DJIRemoteController_DJIRCPIPPosition` Enum.
 *  @param error Error retrieving the value.
 *  @param completion Completion block to receive the result.
 */
...
#pragma mark - Flight mode


/**
 *  Sets the joy stick mode on the remote controller. It is only used when the
 *  remote controller doesn't have a mode switch. Only Supported by Mavic Mini, DJI
 *  Mini SE.
 */
...
/**
 *  Gets the joy stick mode on the remote controller. It is only used when the
 *  remote controller doesn't have a mode switch. Only Supported by Mavic Mini, DJI
 *  Mini SE.
 */
