...
/**
 *  Returns a copy of the current file list on the SD card after you call
 *  `refreshFileListOfStorageLocation:withCompletion`. Returns `nil` if
...
/**
 *  Returns a copy of the current file list on the internal storage after you call
 *  `refreshFileListOfStorageLocation:withCompletion`. Returns `nil` if
 *  `internalStorageFileListState` is reset or unknown.
 *  
 *  @return An array of `DJIMediaFile` objects.
 */
 ...
 *  playing a video,  video data can be received
 *  from`manager:didUpdateVideoPlaybackData:length:forRendering` and playback state
