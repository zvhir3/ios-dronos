...
@class DJIWaypointMissionInterruption;
 ...
 *  Waypoint Missions. It can be accessed from `DJIMissionControl`. For Matrice 300
 *  RTK,  it is recommended that you use `DJIWaypointV2MissionOperator`.
 *  `DJIWaypointMissionOperator` Will no longer be maintained.
 */
...
/**
 *  Gets the interruption of the last waypoint mission. If a waypoint mission is
 *  interrupted  before the completion, the aircraft will record the interruption
 *  information. This information  is only available when the last waypoint mission
 *  is incomplete and the first waypoint of the  mission has been reached. The
 *  interruption will also be recorded if the mission is stopped by  the user. The
 *  interruption information is recorded in the aircraft and the information
 *  persists  even after the aircraft's reboot. Supported only by Phantom 4 RTK.
 *  
 *  @param interruption The interruption information to get. Returns NULL if there is any error.
