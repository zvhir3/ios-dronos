 ...
 *  
 *  @param fc Instance of the flight controller for which the data will be updated.
 *  @param gravityCenterState DJIGravityCenterState object.
 */
 ...
 *  supported by M200 series, M200 V2 series, M300 RTK series, Mavic 2 Pro, Mavic 2
 *  Zoom, Mavic 2 Enterprise, Mavic 2 Enterprise Dual.
 *  
 *  @param fc Instance of the flight controller for which the data will be updated.
...
/**
 *  Setting a larger value increases the yaw speed of the aircraft when the control
 *  sticks are pressed. The settable range of speed changes according to the mode.
 *  When the mode is normal, the speed range is (20,90). When the mode is sport, the
 *  speed range is (20,130). When the mode is tripod, the speed range is (20,30).
 *  Only supported by Mavic Air 2, DJI Air 2S.
 *  
 *  @param speed The speed to set for aircraft heading turning.
 *  @param mode The mode that the setting will be applied to.
 *  @param completion Completion block to receive the result.
 */
...
/**
 *  Gets the heading turning speed of the aircraft according to the mode. Only
 *  supported by Mavic Air 2, DJI Air 2S.
 *  
 *  @param mode The mode to query.
 *  @param speed Gets the heading turing speed.
...
/**
 *  Setting a larger angle increases the yaw buffer when the control sticks are
 *  pressed. The value should be in the range [1, 100]. Only supported by Mavic Air
 *  2, DJI Air 2S.
 *  
 *  @param smoothFactor The smoothness that the setting will be applied to.
 *  @param mode The mode that the setting will be applied to.
 *  @param completion Completion block to receive the result.
 */
...
/**
 *  Gets the smoothness of the aircraft heading turn according to the mode. Only
 *  supported by Mavic Air 2, DJI Air 2S.
 *  
 *  @param mode The mode to query.
 *  @param smoothFactor Gets the smoothness value.
 *  @param completion The completion block that receives the execution result.
 */
...
/**
 *  Starts aircraft precision takeoff. Precision Takeoff is considered complete when
 *  the aircraft is hovering 6 meters (20 feet) above the ground. Completion block
 *  is called when aircraft crosses 0.5 meters (1.6 feet). If the motors are already
 *  on, this command cannot be executed.
...
/**
 *  Enables urgent stop mode. When urgent stop mode is enabled, the motor can be
 *  stopped by pushing sticks inner/outer simultaneously at any time. Use with
 *  caution. Supported only by Phantom 4 RTK.
 *  
 *  @param enabled `YES` to enable urgent stop mode.
 *  @param completion Completion block to receive the result.
 */
 ...
 *  
 *  @param enabled `YES` if urgent stop mode is enabled.
...
/**
 *  When GPS signal low, aircraft unable to auto hover and takeoff restricted. If
 *  you want to unlock takeoff without GPS , ensure there no people or obstacles
 *  within 2 m around the aircraft, and pay close attention to operating the remote
 *  controller at all times to avoid collisions.
 *  
 *  @param enabled `NO` to unlock takeoff without GPS.
 ...
 *  
 *  @param enabled Return `NO` if unlock takeoff without GPS.
 */
 ...
 *  altitude. The valid range for the altitude is from 20m to 500m. But if the
 *  aircraft is M300 RTK, the limit can beyond 500m.
 *  
 ...
 *  aircraft is lost. It is supported by Mavic Mini, DJI Mini 2, DJI Mini SE and
 *  Maivic Air 2.
 *  
 ...
 *  in the range of [15, 50]. It is not supported by Mavic Mini, DJI Mini 2, DJI
 *  Mini SE, Mavic Air 2 and DJI Air 2S.
 *  
 ...
 *  `getLowBatteryWarningThresholdWithCompletion` minus 5. It is supported by Mavic
 *  Mini, DJI Mini 2, DJI Mini SE.
 *  
 ...
 *  completing the go-home action. For Mavic Mini, DJI Mini 2, DJI Mini SE, the
 *  aircraft will return to home point when the remaning battery is low.
 *  
...
/**
 *  Do not turn on the Propeller Cage switch if propeller cages are not mounted. By
 *  enabling the Propeller Cage switch, you will set the Force Landing battery level
 *  to 15% and disable Wind Check and Propeller Check. When propeller cages are
 *  installed, no other accessories (except for official ND filters) can be
 *  attached. Only supported by Mavic Mini, DJI Mini 2, DJI Mini SE.
 *  
 *  @param enabled `YES` to enable propeller cage protection.
 *  @param completion Completion block that receives the execution result.
 */
 ...
 *  
 *  @param enabled `YES` if the propeller cage protection is enabled.
 ...
 *  cancel the request or wait for the countdown to start go-home action. It is
 *  supported by Mavic Mini, DJI Mini 2, DJI Mini SE, DJI Air 2S and Mavic Air 2.
 *  
 ...
 *  IMU calibration. It is recommended to reboot the device after IMU calibration.
 *  It is recommended to invoke `rebootDroneWithCompletion` after IMU calibration.
 *  
...
/**
 *  Starts gravity center calibration. Keep the aircraft hover without wind during
 *  calibration. The completion block will be called once the calibration is
 *  started. Use the `flightController:didUpdateGravityCenterState` method to check
 *  the execution status of the gravity center calibration.
 *  
 *  @param completion Completion block to check if the calibration starts successfully.
 */
 ...
 *  user and disable Tripod Mode. Not supported by Mavic 2 Enterprise Advanced.
 *  
...
/**
 *  Gets tripod mode status (enabled/disabled). It is supported by Mavic Pro, Mavic
 *  Mini, DJI Mini 2, DJI Mini SE.
 *  
 ...
 *  can shorten the braking distance in cinematic mode. It's supported by Mavic Pro,
 *  Spark, Mavic 2 Zoom and Mavic 2 Pro.
 *  
 *  @param brakeSensitivity Brake sensitivity in cinematic mode.
 *  @param error Error retrieving the value.
 ...
 *  available when the aircraft is flying at least 3m above the ground. It is only
 *  supported by Inspire 2.
 *  
...
/**
 *  Gets current Auto Quick Spin mode (enabled/disabled). It is only supported by
 *  Inspire 2.
 *  
...
/**
 *  Reboot the core component of the aircraft. It is recommended to invoke this
 *  method after IMU calibration.
...
/**
 *  `YES` to change the Tripod(T) mode toggle to Attitude mode toggle from the
 *  remote controller. You should called
 *  `setMultipleFlightModeEnabled:withCompletion` to enable multiple flight mode
 *  first, otherwise this interfaces take no effect. if this mode is enabled, once
 *  you switch the mode toggle on remote controller to Tripod(T) mode, aircraft will
 *  enter Attitude mode, in this mode, aircraft is easily distured and drifts
 *  horizontally in consequence, you have to manully hover the aircraft. Also, the
 *  active braking system is inactive. It is supported only by Matrice 300 RTK.
 *  
 *  @param isOpen `YES` to change the Tripod(T) mode toggle to Attitude mode toggle from the remote controller.
 */
- (void)toggleAttitudeFlightMode:(BOOL)isOpen withCompletion:(DJICompletionBlock)completion;


/**
 *  `YES` if custom the Tripod(T) mode to Attitude mode on the remote controller is
 *  enabled. It is supported only by Matrice 300 RTK.
 */
- (void)getIsAttitudeFlightModeOpen:(void (^_Nonnull)(BOOL isAttitudeFlightModeOpen, NSError *_Nullable error))completion;


/**
 *  Enables the coordinated turn. Improves aerodynamic efficiency. The aircraft is
 *  flying in the direction of the turn, without sideslipping. Only supported by
 *  M300 RTK.
 *  
 *  @param enabled `YES` to enable the coordinated turn.
 *  @param completion Completion block that receives the execution result.
 */
 ...
 *  
 *  @param enabled `YES` if coordinated turn  is enabled.
 *  @param error Error if there is any.
 *  @param completion Completion block that receives the execution result.
 */
...
/**
 *  The selection of satellite positioning system is only supported when the RTK
 *  positioning function is turned off, and cannot be modified in the air. If RTK
 *  positioning function is turned on, aircraft uses GPS, GLONASS, Galileo, and BDS
 *  data to determine location. Only supported by M300 RTK.
 *  
 *  @param system Navigation satellite system.
 *  @param completion Completion block that receives the execution result.
 */
...
/**
 *  Enables the power supply port. It is only supported by M200 series and M200
 *  series v2.
 *  
 *  @param enabled `YES` if power supply port is enabled.
 *  @param completion Completion block that receives the execution result.
 */
...
/**
 *  Determines whether the power supply port is enabled or not. It is only supported
 *  by M200 series and M200 series v2.
 *  
 *  @param enabled `YES` if the power supply port is enabled.
 *  @param error Error if there is any.
 *  @param completion Completion block that receives the execution result.
 */
 ...
 *  information about flight orientation. Not supported by Mavic Mini, DJI Mini 2,
 *  DJI Mini SE and Mavic Air 2, DJI Air 2S.
 *  
