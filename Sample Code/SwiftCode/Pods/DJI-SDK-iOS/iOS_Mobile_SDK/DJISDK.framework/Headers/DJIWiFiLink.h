...
/**
 *  It is not supported by Spark, Mavic Air and DJI Mini 2. For these products, set
 *  different channel  numbers to select the frequency band.
 *  
...
/**
 *  Sets the WiFi channel. `setChannelNumber:withCompletion`  must be used to
 *  determine which channels are possible to set. When a new channel is set,  the
 *  WiFi on the product will reboot. The channel can only be changed when the
 *  product is not flying. Supported by Mavic Pro, Mavic Mini, DJI Mini SE.
 *  
 ...
 *  available. Supported only by Mavic Pro, Mavic Mini, DJI Mini SE.
 *  
