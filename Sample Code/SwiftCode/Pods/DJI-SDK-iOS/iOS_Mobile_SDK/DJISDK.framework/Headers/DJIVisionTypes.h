...
/**
 *  The detected obstacle distance to the aircraft in meters. The left and right
 *  obstacle avoidance system is a monocular vision system,  so it can only detect
 *  whether there are obstacles. 0 represents no obstacles and 1 represents
 *  obstacles.  For mavic 2 series, it only supports returning this value in T mode.
 */
 ...
 */
typedef NS_ENUM(NSInteger, DJIFillLightMode) {

    /**
     *  The auto mode. In this mode, the auxiliary light will be turned on automatically
     *  when the downward or top vision system is active but the environment is too
     *  dark.
     */
    DJIFillLightModeAuto,
     ...
     *  behavior  as ''DJIFillLightMode_Auto''.
     *  - When the aircraft ascends above 5.5 meters, the auxiliary light starts to
     *  blink in  1Hz.
     *  - When the aircraft descends below 4.5 meters, the auxiliary light will have the
     *  same behavior as ''DJIFillLightMode_Auto''. This mode is only  supported by
     *  Mavic 2 Enterprise.
     */
    DJIFillLightModeBeacon,


    /**
     *  Unknown.
     */
    DJIFillLightModeUnknown = 0xFF,
};
